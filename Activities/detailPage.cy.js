describe('Activities Detail Page', () => {
  ...
  it('Share with Facebook', () => {
    ...
    cy.get('#trv-share-modal > #trv-modal-content')
      .should('be.visible')
    // The Facebook option is displayed
    cy.get('.social-wrapper > :nth-child(1) > .t-display-table')
      .should('be.visible')
    cy.get(':nth-child(1) > .t-display-table > .social-icon')
      .should('be.visible')
    // Click the 'Facebook' option
    cy.get('.social-wrapper > :nth-child(1) > .t-display-table')
      .click()
    // The button 'Facebook' has link to Facebook page
    cy.get('.social-wrapper > :nth-child(4) > .t-display-table')
      .should('have.attr', 'href', 'javascript:;')
  })

  it('Share with Twitter', () => {
    cy.get('[aria-label="Share"]')
      .click()
    // The Twitter option is displayed
    cy.get('.social-wrapper > :nth-child(3) > .t-display-table')
      .should('be.visible')
    cy.get(':nth-child(3) > .t-display-table > .social-icon')
      .should('be.visible')
    // Click the 'Twitter' option
    cy.get('.social-wrapper > :nth-child(3) > .t-display-table')
      .click()
    // The button 'Twitter' has link to Twitter page
    cy.get('.social-wrapper > :nth-child(4) > .t-display-table')
      .should('have.attr', 'href', 'javascript:;')
  })
  ...
  it('Share with Email', () => {
    ...
    cy.get('.social-wrapper > :nth-child(4) > .t-display-table')
      .should('be.visible')
    cy.get(':nth-child(4) > .t-display-table > .social-icon')
      .should('be.visible')
  ...
  it('Share with Copied link', () => {
    ...
    cy.get('[class="copy-input t-input-text-tinted t-input-text-tinted-left t-input-text-large"]')
      .should('have.attr', 'type', 'text')
    cy.get('.t-input-group > .t-input-icon-group')
      .should('be.visible')
    // Click the 'Copy Link' option
    cy.get('#copylink')
      .click()
    // The 'Link Copied' tooltip is displayed
    cy.get('[class="copy-link-tooltip t-border-radius t-color-white t-m-t-8 active"]')
      .should('exist')
      .and('be.visible')
    cy.get('[class="copy-link-tooltip t-border-radius t-color-white t-m-t-8"]')
      .should('contain', 'Link copied!')
    //  Then the 'Link Copied' tooltip is displayed
    cy.get('[class="copy-link-tooltip t-border-radius t-color-white t-m-t-8"]')
